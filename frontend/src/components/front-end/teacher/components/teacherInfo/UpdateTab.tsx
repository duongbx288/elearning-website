...
const UpdateTab = ({ teaId, teacherInfo, setUpdate, setOpen }) => {
  const [name, setName] = useState<string>(teacherInfo.name ? teacherInfo.name : '');
  const [phoneNumber, setPhoneNumber] = useState<string>(teacherInfo.phoneNumber ? teacherInfo.phoneNumber : '');
  const [birthDate, setBirthDate] = useState<Date>(teacherInfo.birthDate);
  const [address, setAddress] = useState<string>(
    teacherInfo.address ? teacherInfo.address : ''
  );
  const [city, setCity] = useState<string>(teacherInfo.city ? teacherInfo.city : '');

  const [title, setGender] = useState<string>(
    teacherInfo.title ? teacherInfo.title : ''
  );
  ...
  const handleUpdate = () => {
    const update = {
      ...
      teacherCode: 'teacher_' + teaId + name.replace(/\s+/g, '').replace(/[^\x00-\x7F]/g, ""),
      id: teaId,
      name: name,
      birthDate: birthDate,
