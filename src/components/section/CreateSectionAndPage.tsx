...
const CreateSectionAndPage: React.FC = () => {
  ...
  const [errInternal, setErrInternal] = useState(false);
  ...
  const handleKeyDownSection = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.isTrusted) {
      if (e.repeat) {
        return;
      }
  ...
  const savePageInfo = (): void => {
    if (pageUrl.length <= 0 || pageName.length <= 0) {
      setErrOpen1(true);
      return;
    }
    if (
      ...
      PageService.savePage(PageInfo, setErrOpen3).then((response) => {
  ...
  const saveSection = (event: any) => {
    ...
    if (
      ...
      SectionService.save(sectionItem, setErrOpen).then((response) => {
  ...
  return (
    <div>
      ...
      <Snackbar open={openDiv} autoHideDuration={3000} onClose={handleCloseDiv} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
          <Alert severity="success" sx={{ width: '500px', height: '50px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>
           Thêm thành công!
        </Alert>
      </Snackbar>
      <Snackbar open={errOpen} autoHideDuration={3000} onClose={handleCloseDiv} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
          <Alert severity="error" sx={{ width: '500px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>
          Thêm thất bại
        </Alert>
      </Snackbar>
      <Snackbar open={errInternal} autoHideDuration={3000} onClose={handleCloseDiv} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
          <Alert severity="error" sx={{ width: '500px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>
          Có lỗi xảy ra (500)
        </Alert>
      </Snackbar>
      <Snackbar open={errOpen1} autoHideDuration={3000} onClose={handleCloseDiv} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
          <Alert severity="error" sx={{ width: '500px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>
          Bạn cần nhập đầy đủ thông tin trước.
        </Alert>
      </Snackbar>
      <Snackbar open={errOpen2} autoHideDuration={3000} onClose={handleCloseDiv} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
          <Alert severity="error" sx={{ width: '500px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>
          Kích thước của section không hợp lệ
        </Alert>
      </Snackbar>
      <Snackbar open={errOpen3} autoHideDuration={3000} onClose={handleCloseDiv} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
          <Alert severity="error" sx={{ width: '500px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>
          Page URL đã tồn tại
        </Alert>
      </Snackbar>
