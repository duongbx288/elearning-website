...
const BasicInfos = ({ setBInfo , BInfo }) => {
  const { id } = useParams();

  const [category, setCategory] = useState<TypeResponse[]>([]);
  const [name, setName] = useState<string>(BInfo?.name); // ten khoa hoc
  const [description, setDescription] = useState<string>(BInfo?.description);
  const [introduction, setIntroduction] = useState<string>(BInfo?.introduction);
  const [price, setPrice] = useState<string>(BInfo?.price);
  const [typeId, setTypeId] = useState<string>(BInfo?.typeId);
  const [link, setLink] = useState<string>(BInfo?.link); // Video gioi thieu
