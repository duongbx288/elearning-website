import {
  ...
  const CustomCourseDialog = (props: CustomDialogProps) => {
    ...
    let success: Course[];
    let failed: { course: Course; error: string }[] = [];
    ...
    const updateEntities = async (index: number) => {
      if (listEntities && listEntities.length > 0 && index <= listEntities.length - 1) {
        ...
        if (entity && entity.id) {
          ...
          try {
            await CourseService.updateCourseStatus(request);
            success.push(entity);
            console.log('okay?');
          } catch (error) {
            failed.push({ course: entity, error: getMessageError(error) });
    ...
    const processStatusUpdate = (status: string) => {
      if (status === 'active') {
        return 'Kích hoạt khóa học.';
      } else if (status === 'inactive') {
        return 'Tạm dừng khóa học.';
      } else if (status === 'deleted') {
        return 'Hủy bỏ khóa học.';
      }
    };
